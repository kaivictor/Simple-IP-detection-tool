...
def Initialization():
    global AllIp,AllErrorIp,AllTestIpText,Filepath,IpPingIngText,ErrorIpText,TipEntryText,Start
    AllIp = []
    AllErrorIp = []
    AllTestIpText = ''
    IPPingIngText = ''
...
def CmdPing():
    global cmdread,IPPingIngText,AllTestIpText
    for i in AllIp:
        AllTestIpText=("%s\n%s"%(AllTestIpText,i))
    for ip in AllIp:
        IPPingIngText = ('刚刚检测的地址%s'%ip)
...
def Judge(cmdread,ip):
    ...
    if state == []:
        ...
        if speed == []:
            print("检测的地址：%s,存在异常"%(ip))
            AllErrorIp.append(ip)
        else:
            print("检测的地址：%s,速度为：%s"%(ip,speed))
    else:
        AllErrorIp.append(ip)
...
def StratCmd():
    global Start,Filepath,AllTestIpText,TipEntryText,win,AllIp,AllIpText,AllErrorIpText
    Initialization()
    Start['text'] = "稍等"
    Start['state']='disable'
    AllTestIpText=("计划中：")
    ...
    if PatternMultiple['state']=='normal':
        try:
            ...
            tkinter.messagebox.showwarning('错误','您需要填写合适的地址')
        AllIpText=("%s\n%s"%(AllTestIpText,i))
        Update()
...
def Update():
    global IPWillBeTested,AllIpText,IPPing,IPPingIngText,AllIp,AllErrorIpText,AllTestIpText
    IPWillBeTested.delete(1.0, END)
    IPWillBeTested.insert('end',AllTestIpText)
    IPPing['text']=IPPingIngText
    for d in AllErrorIp:
        AllErrorIpText=("%s\n%s"%(AllErrorIpText,d))
...
def SaveResults():
    global AllErrorIpText,AllTestIpText
